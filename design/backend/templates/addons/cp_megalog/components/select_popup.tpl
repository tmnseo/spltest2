            ...
            {if $items_status}
                {foreach from=$items_status item="val" key="st"}
                    <li {if $status == $st}class="disabled"{/if}>
                        <a class="cm-dialog-opener cm-dialog-auto-size {if $text_wrap}dropdown--text-wrap{/if} {if $confirm}cm-confirm {/if}status-link-{$st|lower} {if $status == $st}active{else}cm-ajax{/if} {if $status_meta}{$status_meta}{/if}" href="{"orders.cp_ml_update_status?id=`$id`&status=`$st`&res_ids=`$status_target_id``$extra_params``$dynamic_object`"|fn_url}" id="cp_change_status_{$id}_{$st}" data-ca-target-id="content_cp_change_status_{$id}_{$st}" onclick="return fn_check_object_status(this, '{$st|lower}', '{if $statuses}{$statuses[$st].params.color|default:''}{/if}');" data-ca-event="ce.update_object_status_callback" data-ca-dialog-title="{__("cp_ml_leave_a_comment")}" title="{__("cp_ml_leave_a_comment")}">{$val}</a>
                    </li>
                {/foreach}
            {/if}
