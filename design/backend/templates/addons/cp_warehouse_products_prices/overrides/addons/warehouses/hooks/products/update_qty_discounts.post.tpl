            ...
            <tbody>
                {foreach $warehouses as $warehouse}
                    <tr class="cm-row-item cm-row-status-{$warehouse.status|strtolower}">
                        ...
                        <td data-th="{__("price")}">
                            {if $warehouse_prices[$warehouse.warehouse_id]['price']}
                                {$price = $warehouse_prices[$warehouse.warehouse_id]['price']}
                            {/if}
                            <input type="text" name="product_data[cp_warehouse_products_prices][{$warehouse.warehouse_id}]" value="{$price|default:"0.00"|fn_format_price:$primary_currency:null:false}" class="input-small"/></td>
                    </tr>
                {/foreach}
            </tbody>
        </table>
