...
<div class="table-responsive-wrapper">
    ...
    <tr>
        ...
        <th width="40%"><a class="cm-ajax" href="{"`$c_url`&sort_by=order_id&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__("id")}{if $search.sort_by == "order_id"}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
    ...
    {if $addons.cp_statuses_rules.status == 'A' && $auth.user_type == 'V'}
        {$order_statuses = $o.order_id|fn_cp_get_statuses:$smarty.const.STATUSES_ORDER:$statuses:true:true}
        {$order_status_descr = $o.order_id|fn_cp_get_simple_statuses:$smarty.const.STATUSES_ORDER:false:true}
    {/if}
