...
{capture name="cp_actual_address"}

    {hook name="order_management:cp_actual_address"}
    {if $order_info.cp_address_data.C}
        {$user_actual_address = $order_info.cp_address_data.C}
    {elseif $user_data.cp_address_data.C}
        {$user_actual_address = $user_data.cp_address_data.C}
    {/if}
    {if !fn_is_empty($user_actual_address)}
...
<div class="sidebar-row">
    ...
    {if $order_info.cp_recipient_data}
        <h6>{__("recipient_information")}</h6>
        <div class="profile-info">
            <i class="icon-user"></i>
            <div class="clear">
                {if $order_info.cp_recipient_data.firstname}
                <div>
                    <span>{__("first_name")}:</span>
                    <span>{$order_info.cp_recipient_data.firstname}</span>
                </div>
                {/if}
                {if $order_info.cp_recipient_data.lastname}
                <div>
                    <span>{__("last_name")}:</span>
                    <span>{$order_info.cp_recipient_data.lastname}</span>
                </div>
                {/if}
                {if $order_info.cp_recipient_data.middlename}
                <div>
                    <span>{__("cp_middle_name")}:</span>
                    <span>{$order_info.cp_recipient_data.middlename}</span>
                </div>
                {/if}
                {if $order_info.cp_recipient_data.phone}
                <div>
                    <span>{__("phone")}:</span>
                    <a href="tel:{$order_info.cp_recipient_data.phone}"><bdi>{$order_info.cp_recipient_data.phone}</bdi></a>
                </div>
                {/if}
            </div>
        </div>
...
{if $profile_fields.B}
    ...
    <h6>{__("cp_customer_legal_address")}</h6>
    <div class="profile-info">
        <i class="icon-tag"></i>
...
<div class="sidebar-row">
    ...
    <h6>{__("cp_customer_actual_address")}</h6>
    <div class="profile-info">
        {$smarty.capture.cp_actual_address nofilter}
    </div>
</div>
<hr class="profile-info-delim" />
