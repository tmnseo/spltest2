...
<input type="hidden" name="result_ids" value="content_general,cp_totals_table" />
...
<div id="content_general">
    <div class="row-fluid">
        <div class="span8">
            ...
            {if $order_info.warehouse_data_points}
                {foreach from=$order_info.warehouse_data_points item="warehouse_point"}

                    <div class="control-group" >
                        <span> {$warehouse_point.name} {$warehouse_point.pickup_address}  {$warehouse_point.pickup_phone}</span>
                    </div>
            ...
            <div class="table-responsive-wrapper">
                ...
                <thead>
                    <tr>
                        ...
                        <th>{__('cp_checkout_modifications_manufacturer')}</th>
                        <th>{__('cp_checkout_modifications_manufacturer_code')}</th>
                ...
                <tr>
                    ...
                    <td data-th="{__("cp_checkout_modifications_manufacturer")}">
                        {$oi.manufacturer}
                    </td>
                    <td data-th="{__("cp_checkout_modifications_manufacturer_code")}">
                        {$oi.manufacturer_code}
                    </td>
            ...
            <div class="clearfix">
                <div class="table-wrapper" id="cp_totals_table">
                    <table class="pull-right">
                        ...
                        {if $order_info.display_shipping_cost|floatval}
                            <tr> 
                                ...
                                <td class="right {if $auth.user_type == 'V'}cp-td-shipping-price{/if}" data-ct-totals="shipping_cost"><span id="cp_shipping_price_label">{include file="common/price.tpl" value=$order_info.display_shipping_cost}</span> 
                                    {if $auth.user_type == 'V'}
                                        {if $cp_allowed_change_shipping && in_array($order_info.status, $cp_allowed_change_shipping) && $order_info.cp_confirm_status == 'N'}
                                            <span><a id="cp_change_shipping_cost">{__("change")}</a></span>
                                            <span id="cp_shipping_price_input" class="hidden" data-ca-switch-id="manual_shipping_{$group_key}_{$shipping_key}">
                                                ...
                                                {/if}
                                                {include file="common/price.tpl" value=$stored_shipping_cost view="input" input_name="cp_stored_shipping_cost" class="input-small cp-price-input"}
                                            </span>
                                        {/if}
                                    {/if} 
                                </td>
                            ...
                            {if $auth.user_type == 'V'}
                                <tr class="hidden cm-om-totals-recalculate">
                                    <td>&nbsp;</td>
                                    <td colspan="2">
                                        <button class="btn cm-ajax" type="submit" name="dispatch[orders.cp_update_totals]"><i class="icon-refresh"></i> {__("recalculate_totals")}</button>
                                    </td>
                                </tr>
                            {/if}
                            {if $addons.cp_edost_improvement.status == 'A' && in_array($order_info.status, $cp_allowed_change_shipping) && $order_info.cp_confirm_status == 'N'}
                                <tr>
                                    <td colspan="2">
                                        <a class="cm-dialog-opener cm-dialog-auto-size" href="{"cp_edost_improvement.calculation_popup?order_id=`$order_info.order_id`"|fn_url}" data-ca-target-id="cp_edost_improvement_calculation_popup">{__("cp_edost_improvement.calculate_shipping_cost_with_edost")}</a>
                                    </td>
                                    <td>&nbsp;</td>
                                </tr>
                            {/if}
                        {elseif $addons.cp_edost_improvement.status == 'A' && in_array($order_info.status, $cp_allowed_change_shipping) && $order_info.cp_confirm_status == 'N'}
                            <tr>
                                <td colspan="2">
                                    <a class="cm-dialog-opener cm-dialog-auto-size" href="{"cp_edost_improvement.calculation_popup?order_id=`$order_info.order_id`&is_only_save=1"|fn_url}" data-ca-target-id="cp_edost_improvement_calculation_popup">{__("cp_edost_improvement.enter_order_size")}</a>
                                </td>
                                <td>&nbsp;</td>
                            </tr>
                        {/if}
            ...
            <div class="note clearfix">
                <div class="span6">
                    ...
                    <textarea class="span12 cp-save" name="update_order[notes]" id="notes" cols="40" rows="5">{$order_info.notes}</textarea>
                </div>
                <div class="span6">
                    <label for="details">{__("staff_only_notes")}</label>
                    <textarea class="span12 cp-save" name="update_order[details]" id="details" cols="40" rows="5">{$order_info.details}</textarea>
                </div>
            </div>
        ...
        <div class="span4">
            <div class="well orders-right-pane form-horizontal">
                <div class="control-group">
                    ...
                    <div class="controls" id="cp_statuses_content">
                        {hook name="orders:order_status"}
                            ...
                            {include file="common/select_popup.tpl" suffix="o" id=$order_info.order_id status=$order_info.status items_status=$order_status_descr update_controller="orders" notify=true notify_department=true notify_vendor=$notify_vendor status_target_id="cp_statuses_content,content_downloads,cp_totals_table,cp_planned_time_issuing_order_div" extra="&return_url=`$extra_status`" statuses=$order_statuses popup_additional_class="dropleft" text_wrap=true }

                            {if $auth.user_type == 'V' && $order_info.status && $cp_before_completed_statuses && in_array($order_info.status, $cp_before_completed_statuses)}
                                {include file="addons/cp_edost_improvement/components/size_before_completed.tpl" order_id=$order_info.order_id}
                            {/if}
                        {/hook}
                    </div>
                <!--cp_statuses_content--></div>
                ...
                <!--select_manager--></div>
                <div class="control-group">
                    <label class="control-label" for="cp_vendor_order_number">{__("cp_vendor_order_number")}</label>
                    <div class="controls">
                        <input id="cp_vendor_order_number" class="input-small cp-save" type="text" name="cp_data[cp_vendor_order_number]" value="{$order_info.cp_vendor_order_number|default:''}" />
                    </div>
                </div>
                {include file="common/subheader.tpl" title=__("cp_order_payment_subheader")}
                <div class="control-group">
                    <label class="control-label" for="cp_payment_order_number">{__("cp_payment_order_number")}</label>
                    <div class="controls">
                        <input id="cp_payment_order_number" class="input-small cp-save" type="text" name="cp_data[cp_payment_order_number]" value="{$order_info.cp_payment_order_number}" />
                    </div>
                </div>
                <div class="control-group">
                    <label class="control-label" for="cp_payment_order_date">{__("cp_payment_order_date")}:</label>
                    <div class="controls">
                        {include file="common/calendar.tpl" date_id="cp_payment_order_date" date_meta="cp-save" date_name="cp_data[cp_payment_order_date]" date_val=$order_info.cp_payment_order_date|default:"" start_year=$settings.Company.company_start_year}
                    </div>
                </div>
                <div class="control-group" >
                    <label class="control-label" for="cp_payment_amount">{__("cp_payment_amount")}({$currencies.$primary_currency.symbol nofilter}):</label>
                    <div class="controls">
                        <input id="cp_payment_amount" class="input-small cp-save" type="text" name="cp_data[cp_payment_amount]" value="{$order_info.cp_payment_amount}" />
                    </div>
                </div>
                <div class="hidden">
                ...
                {hook name="orders:shipping_info"}{/hook}
            </div>
            <div class="well orders-right-pane form-horizontal">
                {* Shipping info *}
                {if $order_info.shipping}
                    <div class="control-group shift-top">
                        {include file="common/subheader.tpl" title=__("cp_issue_of_order")}
                    </div>
                    ...
                    {foreach from=$order_info.shipping item="shipping" key="shipping_id" name="f_shipp"}
                        ...
                        <div class="control-group" id="cp_planned_time_issuing_order_div">
                            <label class="control-label" for="cp_planned_time_issuing_order">{__("cp_planned_time_issuing_order")}:</label>
                            {if $auth.user_type == 'V' && !empty($order_info.status) && !in_array($order_info.status, $cp_allowed_change_planned_time_issuing_order)}
                                {$cp_calendar_extra = "disabled"}
                            {else}
                                {$cp_calendar_extra = ""}
                            {/if}
                            <div class="controls">
                                {include file="common/calendar.tpl" date_id="cp_planned_time_issuing_order" date_meta="cp-save" date_name="cp_data[cp_planned_time_issuing_order]" date_val=$order_info.cp_planned_time_issuing_order|default:"" start_year=$settings.Company.company_start_year extra=$cp_calendar_extra} 
                            </div>
                        <!--cp_planned_time_issuing_order_div--></div>
                        {if $shipping.service_code != 'pickup'}
                            <input type="hidden" id="cp_shipping_id" value="{$shipping.shipping_id}">
                            {if $shipping.shipment_keys}
                                ...
                                {foreach from=$shipping.shipment_keys item="shipment_key"}
                                    ...
                                    <div class="control-group">
                                        ...
                                        <div class="controls" id="cp_tracking_number">
                                            ...
                                            <div class="hidden" id="tracking_number_{$shipment_key}">
                                                <input class="input-small cp-save" type="text" id="cp_tracking_number_ajax" name="update_shipping[{$shipping.group_key}][{$shipment.shipment_id}][tracking_number]" size="45" value="{$shipment.tracking_number}" />
                                                ...
                                                <input type="hidden" id="cp_shipment_id" value="{$shipment.shipment_id}"/>
                                            </div>
                                        <!--cp_tracking_number--></div>
                                    </div>
                                    {/hook}
                                {/foreach}
                                ...
                                <div class="control-group">
                                    ...
                                    <div class="controls" id="cp_tracking_number">
                                       <input class="input-small cp-save" type="text" id="cp_tracking_number_ajax" name="update_shipping[{$shipping.group_key}][{$shipment.shipment_id}][tracking_number]" size="45" value="{$shipment.tracking_number}" />
                                        ...
                                        <input type="hidden" id="cp_shipment_id" value="{$shipment_id}"/>
                                     <!--cp_tracking_number--></div>
                                </div>
                        ...
                        <div class="sidebar-row" id="om_ajax_location_map">
                            <div class="control-group shift-top">
                                {include file="common/subheader.tpl" title=__("cp_order_issue_address")}
                            </div>
                            {if $shipping.cp_is_door_delivery == 'Y'}
                            <div class="control-group">
                                <span>
                                    {if $order_info.s_zipcode}
                                        {$order_info.s_zipcode},&nbsp;
                                    {/if}
                                    {if $order_info.s_city}
                                        {$order_info.s_city},&nbsp;
                                    {/if}
                                    {foreach from=$order_info.cp_address_data.S key=s_field_name item=s_field name="cp_address"}
                                        {if !empty($s_field)}
                                            {if $s_field_name == 's_corp'}{__("cp_corp")}&nbsp;
                                            {elseif $s_field_name == 's_office'}{__("cp_office")}&nbsp;
                                            {elseif $s_field_name == 's_home'}{__("cp_home")}&nbsp;
                                            {/if}
                                            {$s_field}{if !$smarty.foreach.cp_address.last},&nbsp;{/if}
                                        {/if}
                                    {/foreach}
                                </span>
                            </div>
                            {else}
                             <div class="control-group">
                                <span>
                                    {if $order_info.cp_address_data.s_city}
                                        {$order_info.cp_address_data.s_city},&nbsp;
                                    {/if}
                                    {if $order_info.cp_address_data.s_address}
                                        {$order_info.cp_address_data.s_address}
                                    {/if}
                                </span>
                            </div>
                            {/if}
                            {$cp_address_data = $order_info.cp_address_data}
                            {if $cp_address_data.s_country_descr || $cp_address_data.s_city || $cp_address_data.s_address}
                                <div class="cm-geo-map-container cm-aom-map-container"
                                     data-ca-geo-map-language="{$smarty.const.CART_LANGUAGE}"
                                     data-ca-aom-country="{$cp_address_data.s_country_descr}"
                                     data-ca-aom-city="{$cp_address_data.s_city}"
                                     data-ca-aom-address="{$cp_address_data.s_address}"
                                ></div>
                            {else}
                                {__('no_data')}
                            {/if}
            ...
            {hook name="orders:cp_customer_shot_info"}
            {/hook}
        </div>
    </div>
<!--content_general--></div>
...
{capture name="buttons"}
    ...
    {/if}
    <div class="btn-group {if !$hide_checkboxes}btn-hover{/if} dropleft">
