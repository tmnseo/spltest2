...
{if $cp_is_vendor}
   <input type="hidden" name="storefront_id" class="cm-no-hide-input" value="{$storefront_id}"/>
{/if}
{if $taxes}
<div class="table-responsive-wrapper">
    ...
    <tr>
        ...
        {if $cp_is_vendor}
        <th class="center">{__("cp_is_primary_tax")}</th>
        {/if}
    ...
    <tr class="cm-row-status-{$tax.status|lower}" data-ct-tax-id="{$tax.tax_id}">
        ...
        {if $cp_is_vendor}
        <td class="center" data-th="{__("cp_is_primary_tax")}">
            <input type="hidden" name="tax_data[{$tax.tax_id}][cp_is_primary_tax]" value="N" />
            <input type="checkbox" class="cm-no-hide-input cp-special-input" name="tax_data[{$tax.tax_id}][cp_is_primary_tax]" value="Y" {if $tax.cp_is_primary_tax == "Y"}checked="checked" {/if} />
        </td>
        {/if}
...
{/capture}
{if $cp_is_vendor}
   {include file="common/mainbox.tpl" title=__("taxes") content=$smarty.capture.mainbox adv_buttons=$smarty.capture.adv_buttons buttons=$smarty.capture.buttons select_languages=true  select_storefront=true
    show_all_storefront=false selected_storefront_id=$selected_storefront_id}
{else}
