...
{capture name="tabsbox"}
    ...
    <form action="{""|fn_url}" method="post" enctype="multipart/form-data" class="form-horizontal form-edit{if !$allow_save} cm-hide-inputs{/if}" name="store_locations_form{$suffix}">
        ...
        <div id="content_detailed">
            <fieldset>
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input type="text" name="store_location_data[position]" id="elm_position" {if $auth.user_type == 'V'}readonly="readonly"{/if} value="{$store_location.position}" size="3">
                    </div>
                </div>
                ...
                <div class="control-group">
                    <label for="elm_pickup_work_time" class="control-label">{__("store_locator.work_time")}{include file="common/tooltip.tpl" tooltip=__("store_locator.work_time.tooltip")}</label>
                    
                    <div class="controls">
                        <input type="hidden" id="cp_worktime_mask" value="{$addons.cp_warehouses_premoderation.worktime_mask nofilter}">
                        <input class="input-large" type="text" name="store_location_data[pickup_time]" id="elm_pickup_work_time" size="55" value="{$store_location.pickup_time}" placeholder="{$addons.cp_warehouses_premoderation.worktime_placeholder}" />
                    </div>
                </div>

                <div class="control-group {if $auth.user_type == 'V'}hidden{/if}">
                ...
                {if $store_location.status == 'P' || $store_location.status == 'F'}
                <div class="control-group">
                    <label class="control-label">{__("status")}:</label>
                    <div class="controls cp-status">
                {/if}
                
                {if $store_location.status == 'P' && $auth.user_type == 'A'}
                    {include file="addons/cp_warehouses_premoderation/components/premoderation_select_status.tpl" loc_id=$store_location.store_location_id loc_name=$store_location.name}
                    {include file="addons/cp_warehouses_premoderation/components/disapproval_popup.tpl" loc_id=$store_location.store_location_id}
                {elseif $store_location.status == 'F'}
                    <span class="view-status">{__("cp_disapproved")}
                        {if $store_location.cp_disapprove_reason}
                            {include file="common/tooltip.tpl" tooltip=$store_location.cp_disapprove_reason}
                        {/if}
                    </span>
                {elseif $store_location.status == 'P' && $auth.user_type == 'V'}
                    <span class="view-status">{__("pending")}</span>
                {else}
        ...
        {capture name="buttons"}
            {if $auth.user_type == 'V' && $store_location.store_location_id}
                {$dispatch="dispatch[cp_warehouses_premoderation.store_locator_update]"}
            {else}
                {$dispatch="dispatch[store_locator.update]"}
            {/if}
            {if !$id}
                {include file="buttons/save_cancel.tpl" but_name=$dispatch but_role="submit-link" but_target_form="store_locations_form{$suffix}"}
            {else}
                {if !$show_save_btn}
                    {assign var="hide_first_button" value=true}
                    {assign var="hide_second_button" value=true}
                {/if}
                {if $is_premoderation_update != 'Y' || $auth.user_type != 'A'}
                    {include file="buttons/save_cancel.tpl" but_name=$dispatch hide_first_button=$hide_first_button hide_second_button=$hide_second_button but_role="submit-link" but_target_form="store_locations_form{$suffix}" save=$id}
                {/if}
            {/if}
        {/capture}
