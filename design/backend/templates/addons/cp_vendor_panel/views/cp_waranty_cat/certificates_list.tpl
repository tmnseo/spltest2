...
<div class="btn-toolbar clearfix">
    ...
    {capture name="add_new_picker"}
        {include file="addons/cp_vendor_panel/views/cp_waranty_cat/certificate_update.tpl" attachment=[] object_id=$object_id object_type=$object_type}
    {/capture}
    {include file="common/popupbox.tpl" id="add_new_attachments_files" text=__("cp_vp_add_certificate") link_text=__("cp_vp_add_certificate") content=$smarty.capture.add_new_picker act="general" icon="icon-plus"}
    </div>
</div>
{/if}

{if $cp_vp_certificates}
<div class="table-responsive-wrapper">
    <table class="table table-middle table--relative table-objects table-responsive table-responsive-w-titles">
    {foreach from=$cp_vp_certificates item="a"}
        {capture name="object_group"}
            {include file="addons/cp_vendor_panel/views/cp_waranty_cat/certificate_update.tpl" attachment=$a object_id=$object_id object_type=$object_type hide_inputs=$hide_inputs}
        {/capture}
