...
{if $view_mode != "blocks"}
    {capture name="add_buttons"}
        {if $view_mode != "list"}
            ...
            {if $multiple}
                {assign var="_but_text" value=$but_text|default:__("cp_vp_add_category")}
        ...
        {else}
            ...
            {include file="buttons/button.tpl" 
                ...
                but_href="cp_waranty_cat.picker?display=`$display`&data_id=`$data_id`&company_ids=`$company_ids``$extra_url`&selected_category_ids=`$selected_category_ids`"|fn_url 
                but_text=$_but_text 
                but_role=$_but_role 
                but_icon=$_but_icon 
...
{if !$extra_var && $view_mode != "button"}
    ...
    {/if}
        {if $multiple}
            {include file="addons/cp_vendor_panel/pickers/categories/js.tpl" category_id="`$ldelim`category_id`$rdelim`" holder=$data_id hide_input=$hide_input input_name=$input_name radio_input_name=$radio_input_name clone=true hide_link=$hide_link hide_delete_button=$hide_delete_button position_field=$positions position="0"}
        {/if}
        {if $view_mode == "list"}
            {foreach from=$item_ids item="c_id" name="items"}
                {include file="addons/cp_vendor_panel/pickers/categories/js.tpl" main_category=$main_category category_id=$c_id holder=$data_id hide_input=$hide_input input_name=$input_name clone=false hide_link=$hide_link first_item=$smarty.foreach.items.first view_mode="list"}
            {foreachelse}
                {include file="addons/cp_vendor_panel/pickers/categories/js.tpl" category_id="" holder=$data_id hide_input=$hide_input input_name=$input_name clone=true hide_link=$hide_link view_mode="list"}
            {/foreach}
        {else}
            {foreach from=$item_ids item="c_id" name="items"}
                {if !$multiple}<div class="input-append choose-input">{/if}
                {include file="addons/cp_vendor_panel/pickers/categories/js.tpl" category_id=$c_id holder=$data_id hide_input=$hide_input input_name=$input_name hide_link=$hide_link hide_delete_button=$hide_delete_button first_item=$smarty.foreach.items.first position_field=$positions position=$smarty.foreach.items.iteration+$start_pos}
                {if !$multiple}</div>{/if}<!-- /.choose-input -->
            {foreachelse}
                {if !$multiple}
                <div class="input-append choose-input">
                    {include file="addons/cp_vendor_panel/pickers/categories/js.tpl" category_id="" holder=$data_id hide_input=$hide_input input_name=$input_name hide_link=$hide_link hide_delete_button=$hide_delete_button}
                    {$smarty.capture.add_buttons nofilter}
                </div>
                {/if}
            {/foreach}
