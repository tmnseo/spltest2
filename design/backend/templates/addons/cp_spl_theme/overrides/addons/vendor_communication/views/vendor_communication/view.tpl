                ...
                <div id="new_message_{$thread_id}" class="add_message_form--wrapper">
                    ...
                    <div class="buttons-container">
                        ...
                        {if $thread.object && $thread.object_type == $smarty.const.VC_OBJECT_TYPE_PRODUCT}
                            {$storefront_id = $thread.user_id|fn_cp_get_user_storefront_id}
                            {if !empty($thread.warehouse_id)}
                                 <a target="_blank" href={"products.view?product_id=`$thread.object.product_id`?warehouse_id={$thread.warehouse_id}&storefront_id=$storefront_id"|fn_url:C}
                                    class="post-object" title="{$thread.object.product}">
                                    {$thread.object.product}
                                </a>
                            {else}
                                 <a target="_blank" href={"products.view?product_id=`$thread.object.product_id`&storefront_id=$storefront_id"|fn_url:C}
                                    class="post-object" title="{$thread.object.product}">
                                    {$thread.object.product}
                                </a>
                            {/if}
