...
<div class="table-responsive-wrapper">
    ...
    <tr>
        ...
        <th {if $auth.user_type == 'V'}width="50%"{else}width="20%"{/if}>{__("name")}</th>
        {if $auth.user_type != 'V'}
    ...
    <input type="hidden" name="shipping_data[{$shipping.shipping_id}][tax_ids][{$shipping.tax_ids}]" value="{$shipping.tax_ids}" />
        ...
        {if $auth.user_type != 'V'}
            ...
            <td class="nowrap" data-th="{__("weight_limit")}&nbsp;({$settings.General.weight_symbol})">
            ...
            {if !"ULTIMATE:FREE"|fn_allowed_for}
                <td class="nowrap" data-th="{__("usergroups")}">
