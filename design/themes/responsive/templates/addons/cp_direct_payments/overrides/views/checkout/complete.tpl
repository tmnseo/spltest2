    ...
    <p>{__("text_order_placed_successfully")}
        {if $order_info}
            {if $order_info.child_ids}
                <a href="{"orders.search?period=A&order_id=`$order_info.child_ids`"|fn_url}" target="block">{__("order_details")}</a>
            {else}
                <a href="{"orders.details?order_id=`$order_info.order_id`"|fn_url}" target="block">{__("order_details")}</a>
            {/if}
        {/if}
    </p>
</div>
