{if $product}
    ...
    {$best_warehouse_id = $product.best_warehouse_id}
    {$price = $product.extra_warehouse_data.$best_warehouse_id.price}
    {$base_price = $product.extra_warehouse_data.$best_warehouse_id.base_price}
    {$discount_percentage = $product.extra_warehouse_data.$best_warehouse_id.discount_prc}
    {$promotions = $product.extra_warehouse_data.$best_warehouse_id.promotions}
    ...
    <div class="ty-simple-list clearfix">
        ...
        {$smarty.capture.$form_open nofilter}
            ...
            {if $discount_percentage}
                {$label_text = "{__("save_discount")} -{$discount_percentage}%"}
                <div class="ty-product-labels ty-product-labels--{if $items.exist_images}top-left{else}top-right{/if}  cm-reload-{$obj_prefix}{$obj_id}" id="product_labels_update_{$obj_prefix}{$obj_id}">
                 {include
                            ...
                            <span class="cp-item-price__value">
                                {if $promotions}
                                    <span class="ty-price_old hidden">
                                        {include file="common/price.tpl" value=$base_price class="ty-list-price"}
                                    </span>
                                {/if}
                                <span class="ty-price_real">
                                    {include file="common/price.tpl" value=$price class="ty-nowrap"}
                                </span>
                ...
                {if $show_add_to_cart}
                    <div class="ty-simple-list__buttons">
                        <div class="cp-np-mosts__1st-line_arrow">
                            <a class="cp-np__switch-from-block" data-ca-target-id="{$block_res_ids}" href="{"products.view?product_id=`$product.product_id`&warehouse_id=`$product.best_warehouse_id`"|fn_url}"><span><i class="icon-spl-arrow-right"></i></span></a>
                        </div>
