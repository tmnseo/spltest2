...
<div class="ty-tabs ty-tabs_spl cm-j-tabs{if $track} cm-track{/if} clearfix cm-j-tabs-disable-convertation">
    <ul class="ty-tabs__list-scroll ty-tabs__list owl-carousel" {if $tabs_section}id="tabs_{$tabs_section}"{/if}>
...
{if $_tabs}
{if $product_wrap}
    <div class="ty-product-detail__content">
{/if}
...
(function(_, $) {
    $.ceEvent('on', 'ce.commoninit', function(context) {
        var elm = context.find('.ty-tabs__list-scroll');
        var widthDocument = $(document).width();

        if (elm.length && widthDocument <= 1160) {
            elm.slick({
                dots: false,
                arrows: true,
                infinite: false,
