...
{capture name="main_icon"}
        {include file="common/image.tpl" obj_id="`$preview_id`_`$image_id`" images=$product.main_pair link_class="cm-image-previewer" image_width=$image_width image_height=$image_height image_id="preview[product_images_`$preview_id`]" data_image_order=$image_counter}
{/capture}
    ...
    <div class="ty-product-img__gallery" data-ca-items-count="3" data-ca-items-responsive="true" id="product_images_{$preview_id}">
        ...
        {foreach from=$product.image_pairs item="image_pair"}
            {if $image_pair}
                ...
                <div class="ty-product-img__gallery-item slide-item">
                    <div>
                        {include file="common/image.tpl" obj_id="`$preview_id`_`$image_id`" no_ids=true images=$image_pair link_class="cm-image-previewer" image_width=$image_width image_height=$image_height image_id="preview[product_images_`$preview_id`]" data_image_order=$image_counter}
                    </div>
                </div>
            {/if}
        {/foreach}
