...
<table class="ty-table ty-vendor-communication-search" id="threads_table">
    <thead>
    <tr>
        <th class="ty-vendor-communication-search__label">&nbsp;</th>
        ...
        {if "MULTIVENDOR"|fn_allowed_for}
            <th><a class="cm-ajax" href="{"`$c_url`&sort_by=company&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-dialog-class="ty-vendor-communication-post__popup" data-ca-target-id={$rev}>{__("writes_to_you")}{if $search.sort_by == "company"}{$sort_sign nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
        {/if}
        <th class="ty-vendor-communication-search__message">{__("message")}</th>
        <th><a class="cm-ajax" href="{"`$c_url`&sort_by=last_updated&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-dialog-class="ty-vendor-communication-post__popup" data-ca-target-id={$rev}>{__("date_application")}{*if $search.sort_by == "last_updated"}{$sort_sign nofilter}{else}{$c_dummy nofilter}{/if*}</a></th>
    ...
    {foreach from=$threads item="thread"}
        ...
        <tr>
            <td class="ty-vendor-communication-search__item ty-vendor-communication-search__label">
                {if $has_new_message}
        ...
        <div id="view_thread_{$thread.thread_id}" class="hidden ty-vendor-communication-view-thread" title="{__("cp.vendor_communication.contact_with", ["[thread_company]" => $thread.company])}"></div>
        {foreachelse}
