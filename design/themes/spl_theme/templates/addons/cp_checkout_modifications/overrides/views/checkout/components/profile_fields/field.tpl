<div class="litecheckout__field cm-field-container {$wrapper_class} {$field_type_class_postfix}"
    ...
    {elseif $field.field_name == 's_address'}
        <input class="litecheckout__input{if $field.class} {$field.class}{/if}{$input_meta} hidden"
               placeholder=" "
               id="{$field_id}"
               type="{$type}"
               name="{$field_name}"
               value="{$field_value}"
               data-ca-lite-checkout-field="{$field_name_helper}"
               data-ca-lite-checkout-auto-save="false"
               autocomplete="{$field.autocomplete}"
               aria-label="{$field.description}"
               title="{$field.description}"
               {$field.attributes|render_tag_attrs nofilter}
        />
    {elseif $field.field_name == 's_street' || $field.field_name == 's_home' || $field.field_name == 's_corp' || $field.field_name == 's_office'}
        <input class="litecheckout__input{if $field.class} {$field.class}{/if}{$input_meta} cp-dadata-correct"
               placeholder=" "
               id="{$field_id}"
               type="{$type}"
               name="{$field_name}"
               ...
               {$field.attributes|render_tag_attrs nofilter}
        />
    {elseif $field.field_name == 's_city'}
        <input class="litecheckout__input{if $field.class} {$field.class}{/if}{$input_meta} cp-dadata-correct"
               placeholder=" "
               id="{$field_id}"
               type="{$type}"
               name="{$field_name}"
               value="{$field_value}"
               data-ca-lite-checkout-field="cp_{$field_name_helper}" {*16.04.2020 gMelnikov modifs*}
               data-ca-lite-checkout-auto-save="false"
               autocomplete="{$field.autocomplete}"
               aria-label=" "
               title="{$field.description}"
    ...
    {else}
        ...
        />
    {/if}
    <label class="{if $field.field_name == 's_address'} hidden {/if}litecheckout__label {$label_meta}" for="{$field_id}">{$field.description}</label>
</div>