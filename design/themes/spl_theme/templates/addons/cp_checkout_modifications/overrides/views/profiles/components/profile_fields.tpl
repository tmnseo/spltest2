...
{/if}
    ...
    {if $show_hint && $field.field_name == "inn"}
        <p class="cp-inn-hint">{$text_hint nofilter}</p>
    {/if}
        {if $field.field_name == "b_address"}
            <div class="ty-profile-field__pseudo-switch">
                <input class="pseudo-switch-address__input hidden" type="checkbox" id="pseudo_switch_address" name="pseudo_switch_address" {if $user_data.cp_is_matching_addresses == 'Y' || $runtime.mode == "add"}checked="checked"{/if}>
