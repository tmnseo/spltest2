...
class Manager
{
    ...
    public function saveProductStock(ProductStock $product_stock)
    {
        ...
        $cp_warehouse_data = db_get_hash_array("SELECT product_id, warehouse_id, price, city_id FROM ?:warehouses_products_amount WHERE product_id = ?i",'warehouse_id', $product_id);
        ...
        if (!empty($cp_warehouse_data)) {
            $warehouses_amounts = array_replace_recursive($warehouses_amounts, $cp_warehouse_data);
        }
        
        foreach ($warehouses_amounts as &$wh_data) {
            
            if (!isset($wh_data['amount'])) {

                $cp_wh_data['amount'] = 0;
                $wh_data = array_merge($cp_wh_data, $wh_data);
            }
            if (!isset($wh_data['price']) || !isset($wh_data['city_id'])) {
                $wh_data = array(
                    'amount' => $wh_data['amount'],
                    'product_id' => $wh_data['product_id'],
