...
class RusEximCommerceml
{
    ...
    public function importFeaturesFile($data_features, $import_params, $data_pos_start, &$import_pos, &$progress)
    {
        ...
        if (isset($data_features -> {$cml['property']})) {
            ...
            foreach ($data_features -> {$cml['property']} as $_feature) {
                ...
                $global_company_id = 0;
                $feature_import = db_get_row("SELECT pf.feature_id as id, pfd.description as name, pf.feature_type as type "
                        . "FROM ?:product_features as pf "
                        . "LEFT JOIN ?:product_features_descriptions as pfd ON pf.feature_id = pfd.feature_id AND pfd.lang_code = ?s "
                        . "WHERE pf.company_id = ?i AND pfd.description = ?s"
                        , $import_params['lang_code'], $global_company_id, $feature_name);
                
                if (empty($feature_import)) {
                    continue;
                }
                
                $features_import[strval($_feature -> {$cml['id']})] = $feature_import;
    ...
    //
    // CP 1C: update feature values
    //
    public function addFeatureValues($variants_data)
    {
        if (!empty($variants_data['variant_id'])) {
            // [cp: avyriev]
            if (is_array($variants_data['variant_id'])) {
                $variants_data['variant_ids'] = $variants_data['variant_id'];
            }
            else {
                $variants_data['variant_ids'] = [$variants_data['variant_id']];
            }
            ...
            $variant_ids = $this->db->getColumn(
                ...
                . " WHERE feature_id = ?i AND product_id = ?i AND variant_id IN (?n)",
                $variants_data['feature_id'],
                $variants_data['product_id'],
                $variants_data['variant_ids']
            );

            if (!empty($variant_ids)) {
                $this->db->query(
                    "DELETE FROM ?:product_features_values"
                    . " WHERE feature_id = ?i AND product_id = ?i AND variant_id NOT IN (?n)",
                    $variants_data['feature_id'],
                    $variants_data['product_id'],
                    $variants_data['variant_ids']
                );
            ...
            foreach (Languages::getAll() as $lang_code => $lang_data) {
                ...
                foreach($variants_data['variant_ids'] as $variant_id) {
    ...
    //
    // CP 1C: change import feature variants
    //
        ...
        if (!empty($data_product -> {$cml['properties_values']} -> {$cml['property_values']})) {
            foreach ($data_product -> {$cml['properties_values']} -> {$cml['property_values']} as $_feature) {
                ...
                if (!empty($features_commerceml[$feature_id]['id'])) {
                    ...
                    // [cp: avyriev]
                    if ($variant_data['feature_type'] != ProductFeatures::MULTIPLE_CHECKBOX) {
                    ...
                    // [cp: avyriev] // FIX IT (after clarification)
                    if ($variant_data['feature_type'] == ProductFeatures::MULTIPLE_CHECKBOX) {
                        $multiple_variants = explode(RUS_EXIM_1C_MULTIPLE_DELIMITER, $variant);
                        if (!empty($multiple_variants)) {
                            foreach ($multiple_variants as $mv) {
                                list($d_variant, $params_variant) = $this->checkFeatureVariant($variant_data['feature_id'], $mv, $import_params['lang_code']);
                                if (!empty($d_variant)) {
                                    $variant_data['variant_id'][] = $d_variant;
                                } else {
                                    $variant_data['variant_id'][] = fn_add_feature_variant($variant_data['feature_id'], array('variant' => $mv));
                                }
                            }
                        }
                    }
    ...
    */
    ...
    protected function updateWarehouse(Warehouse $warehouse)
    {
        ...
        if ($warehouse->getId() == null) {
            ...
            if (fn_allowed_for('ULTIMATE')) {
                fn_ult_update_share_object($store_location_id, 'store_locations', $this->company_id);
            }
        }
    ...
    protected function convertOfferWarehousesStockXmlToOfferWarhouses($offer)
    {
        ...
        foreach ($offer->{$cml['warehouse']} as $warehouse_info) {
            ...
            $offer_warehouse = OfferWarehouse::create($warehouse_id, (int) $warehouse->{$cml['warehouse_in_stock']});

            if ($offer_warehouse->getAmount() < 0) {
                continue;
            }

            $result[] = $offer_warehouse;
        }
