...
class Client
{
    ...
    {
        $response = $this->doGet($this->apiHost, "2/users/get_current_account");
        if ($response->statusCode !== 200) throw RequestUtil::unexpectedStatus($response);
    ...
    function getFile($path, $outStream, $rev = null)
    {
        ...
        $url = $this->buildUrlForGetOrPut(
            $this->contentHost,
            $this->appendFilePath("2/files/download", $path),
            array("rev" => $rev));
...
//             "We sent $len bytes, but server returned an offset of $byteOffset");
    ...
    function chunkedUploadContinue($uploadId, &$byteOffset, $data)
    {
        ...
        $response = $this->_chunkedUpload(
            array('cursor' => array("session_id" => $uploadId, "offset" => $byteOffset), 'close' => false), $data);
    ...
    function chunkedUploadFinish($uploadId, $path, $writeMode, $byteOffset)
    {
        ...
        $params = array(
            "cursor" => array("session_id" => $uploadId, 'offset' => $byteOffset), 
            "commit" => array(
                'path' => $path,
                'mode' => 'add',
                'autorename' => true,
        ...
        $url = $this->buildUrlForGetOrPut(
            $this->contentHost, "2/files/upload_session/finish");
        $curl = $this->mkCurl($url);
        ...
        // Marked 'protected' so I can override it in testing.
    {
        if (empty($params)) {
            $url = $this->buildUrlForGetOrPut(
                $this->contentHost, "2/files/upload_session/start");
            $params = array(
                'close' => false
            );
        } else {
            $url = $this->buildUrlForGetOrPut(
                $this->contentHost, "2/files/upload_session/append_v2");
        }
