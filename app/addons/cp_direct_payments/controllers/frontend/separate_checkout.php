        ...
        foreach ($group_products as $vendor_id => $products) {
            ...
            if (md5(serialize($cart['products'])) != $previous_state && empty($cart['skip_notification'])) {
                ...
                if (!empty($added_products)) {
                    ...
                    $cart_global =  fn_cp_direct_payments_get_mini_cart();
                    $view->assign('cart_global', $cart_global);
                    ...
                    $msg = $view->fetch('addons/cp_direct_payments/views/separate_checkout/components/product_notification.tpl');
        ...
        if (defined('AJAX_REQUEST')) {
            ...
            if (fn_is_empty($cart_products) && fn_is_empty($cart['product_groups'])) {
                ...
                $view->display(
                    ...
                    ? 'addons/cp_direct_payments/views/separate_checkout/components/checkout_totals.tpl'
                    : 'addons/cp_direct_payments/views/separate_checkout/components/shipping_estimation.tpl'
                );
            }
...
if ($mode === 'cart') {
   ...
   // $pr_groups =Tygh::$app['session']['cart']['product_groups'];

   //  unset(Tygh::$app['session']['cp__cart']);


   // $pr_groups = array();
   // foreach ($pr_groups as $com_id => $gr_data){


        //if($gr_data['company_id'] = 11){
            //Tygh::$app['session']['cart']['product_groups']['11_1'] = $gr_data;

            //if(isset( Tygh::$app['session']['cp__cart']['product_groups'])){
               // unset( Tygh::$app['session']['cp__cart']['product_groups']);
            //}
           // Tygh::$app['session']['cp__cart']['11_1'] = $gr_data;
            //$_SESSION['cp__cart']['11_1'] = $gr_data;
           // break;
       // }
  // }
  ...
  function cp_sort_cmp($a, $b)
	{
		if(!isset($a['total']) or !isset($b['total'])){
			return 0;
		}
    ...
    foreach ($carts as $vendor_id => &$cart) {
        ...
        if (!fn_cart_is_empty($cart)) {
            ...
            if(is_array($splitted_ids_company_warehouse) && count($splitted_ids_company_warehouse) ==2){
                ...
                $store_location = fn_get_store_location($warehouse_id, DESCR_SL);

                $cart['store_location_data'] =$store_location;
            ...
            if(!empty($cart['products'])){
                ...
                foreach ($cart['products'] as $key => &$product) {
                    ...
                    $carts[$vendor_id]['products'][$key]['manufacturer_code'] = $product['manufacturer_code'] = db_get_field("SELECT vd.variant FROM ?:product_features_values as v LEFT JOIN ?:product_feature_variant_descriptions as vd ON v.variant_id = vd.variant_id WHERE v.product_id = ?i AND v.feature_id = ?i AND vd.lang_code = ?s", $product['product_id'], $manufacturer_code_id, CART_LANGUAGE);
                    $carts[$vendor_id]['products'][$key]['manufacturer'] = $product['manufacturer'] = db_get_field("SELECT vd.variant FROM ?:product_features_values as v LEFT JOIN ?:product_feature_variant_descriptions as vd ON v.variant_id = vd.variant_id WHERE v.product_id = ?i AND v.feature_id = ?i AND vd.lang_code = ?s", $product['product_id'], $manufacturer_id, CART_LANGUAGE);


                    if(!empty($cart_products[$key])){
                        $cart_products[$key]['manufacturer_code'] = $product['manufacturer_code'];
                        $cart_products[$key]['manufacturer'] = $product['manufacturer'];
                    }

                    if(!empty($product['extra']['warehouse_id'])){

                        $cart_products[$key]['extra']['warehouse_id'] = $product['extra']['warehouse_id'];
                    }
            ...
            $carts_total += floatval($carts[$vendor_id]['display_subtotal']);
    ...
    fn_cp_direct_payments_get_mini_cart();
    ...
    $view->assign([
        ...
        'cp_global_cart_values'  =>Tygh::$app['session']['cp_global_cart_values']
