...
function fn_get_product_exceptions($product_id, $short_list = false)
{
    ...
    if (!isset($cache[$product_id])) {
        ...
        $cache[$product_id] = $exceptions;
    } else {
        $exceptions = $cache[$product_id];
    }
...
function fn_get_products($params, $items_per_page = 0, $lang_code = CART_LANGUAGE)
{
    ...
    if ($params['load_products_extra_data']) {
        ...
        $cp_field = 'product_id';
        fn_set_hook('cp_get_products_set_keys', $products, $params, $lang_code, $group_by, $cp_field);
        
        $products = fn_array_elements_to_keys($products, $cp_field);
    }
    ...
    fn_set_hook('get_products_post', $products, $params, $lang_code);
     if (!empty($params['qqq']) && $params['qqq'] == 'qqq'){
        fn_print_r($products);
    }
