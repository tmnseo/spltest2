...
{$result_ids="cart_items`$suffix_key`,checkout_totals`$suffix_key`,checkout_steps`$suffix_key`,cart_status*,checkout_totals,checkout_totals_header* ,checkout_cart`$suffix_key`"}

<div class="clearfix">
    <form name="cart_form{$suffix_key}"
          id="cart_form{$suffix_key}"
          class="cm-check-changes cm-ajax-full-render"
          action="{""|fn_url}"
          method="post"
    ...
    >
        ...
        <div data-ca-switch-id="cart{$suffix_key}">
            ...
            {include file="addons/cp_direct_payments/views/separate_checkout/components/cart_items.tpl"
