...
{foreach from=$other_list_order item="o_prods"}
    ...
    {if $items.from_others.$o_prod_id && $items.from_others.$o_prod_id.extra_warehouse_data && $items.from_others.$o_prod_id.extra_warehouse_data.$o_wh_id}
        {$cur_prod_data = $items.from_others.$o_prod_id}
        {$skip_this_wh = false}
        ...
        {if $cur_prod_data.extra_warehouse_data.$o_wh_id.amount > 0 && !$skip_this_wh}
            <tr>
                ...
                <td>{include file="common/price.tpl" value=$cur_prod_data.extra_warehouse_data.$o_wh_id.price}</td>
