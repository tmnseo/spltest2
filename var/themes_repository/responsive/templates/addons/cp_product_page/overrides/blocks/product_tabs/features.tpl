        ...
        <div class="ty-product-feature__value">
            ...
            {elseif $feature.feature_type == "ProductFeatures::MULTIPLE_CHECKBOX"|enum && $feature.variants}
                ...
                {foreach from=$feature.variants item="var"}
                    ...
                    {if !$hide_variant_affix}<span class="ty-product-feature__prefix">{$feature.prefix}</span>{/if}{$var.variant}{if !$hide_variant_affix}<span class="ty-product-feature__suffix">{$feature.suffix}</span>{/if}</li>{/if}
                {/foreach}
                </ul>
...
{foreach $product_features as $feature}
    {if $feature.feature_type == "ProductFeatures::GROUP"|enum && $feature.subfeatures}
        <div class="ty-product-feature-group">
            ...
            <div class="ty-product-feature">
                <span class="ty-product-feature__label">{__("weight")}:</span>
                <div class="ty-product-feature__value">{$product.weight}</div>
            </div>
        </div>
    {/if}
{/foreach}